...
namespace ExportV2
{
    public partial class FrmStation : Form
    {
        ...
        public Dictionary<string, string> LoadFiles {
            ...
            {
                return loadFiles
                        .Where(x => x.Key.Contains("lblxls") && !string.IsNullOrEmpty(x.Value))
        ...
        private void FrmStation_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (!InputDataIsOk())
            {
                AcadApp.ShowAlertDialog("Input data inconsistent. See error log");
                e.Cancel = true;
                return;
            }
       ...
       private bool InputDataIsOk()
        {
            ...
            {
                ErrLogger.Log("Station Id or station name missed");
                error = true;
            }
            if (version != "01P01" && !loadFiles.ContainsKey(nameof(lblxlsOrderRdd)))
            {
                ErrLogger.Log("Ordered Rdd missed");
                error = true;
            }

            if (version != "01P01" && !File.Exists(loadFiles[nameof(lblxlsOrderRdd)]))
            {
                ErrLogger.Log("Ordered Rdd not found '"  + loadFiles[nameof(lblxlsOrderRdd)] + "'");
                error = true;
            }
            foreach (KeyValuePair<string, string> loadFile in loadFiles
                                                              .Where(x => x.Key.Contains("lblxls") &&
                                                              !string.IsNullOrEmpty(x.Value) &&
                                                              this.labelsControls[x.Key].Enabled))
            {
                if (!File.Exists(loadFile.Value))
                {
                    ErrLogger.Log("File not found '" + loadFile.Value + "'");
                    error = true;
                }
            }
            return !error;
